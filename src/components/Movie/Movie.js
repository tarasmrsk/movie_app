...
function Movie({ id, posterPath, title, releaseDate, overview, movieGenres, rateMovie }) {
  ...
  const clickStars = (movieId, ratingId) => {
    console.log(`Оценка фильма с ID ${movieId}: ${ratingId}`)
    rateMovie(movieId, ratingId)
  }
  
  const handleRatingChange = (newRating) => {
    setRating(newRating)
    clickStars(id, newRating)
  }
